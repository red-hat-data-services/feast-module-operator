...
func TestMain(m *testing.M) {
	fmt.Fprintf(os.Stderr, "Starting post-upgrade smoke test suite\n")
	...
	// Pre-flight: verify upgraded operator is deployed before running post-upgrade tests
	pods, err := k8sClient.CoreV1().Pods(namespace).List(ctx, metav1.ListOptions{
		LabelSelector: "control-plane=controller-manager,app.kubernetes.io/name=feast-operator",
	})
	if err != nil || len(pods.Items) == 0 {
		log.Fatalf("No feast-operator controller-manager pod found in %s — deploy the upgraded operator before running post-upgrade tests", namespace)
	}
